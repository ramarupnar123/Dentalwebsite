    ...
    <div class="page-wrapper">
        ...
        <main id="js-page-content" role="main" class="page-content" style="padding:0px">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <h2 class="text-center text-info font-weight-bold">Welcome Slider Page</h2>
                    </div>
                    ...
                    <div class="col-md-12 mt-5">
                        <table class="table table-bordered table-hover table-striped table-info">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="slider.php"><button class="btn btn-primary"><i class="fa-solid fa-pen-to-square"></i></button></a>

                                    <a href="slider.php?deleteid=<?=$row['id'];?>&img=<?=$row['img'];?>"><button class="btn btn-success" type="button" onclick="return confirm('are you sure')"><i class="fa-solid fa-trash"></i></button></a>
                                </td>
                            </tr>
                            </form>
                            <?php
...
if(isset($_POST['title'])){
    ...
    $img=image($name,$size,$tmp,$path);
     ...
     $query="INSERT INTO slider(title,descp,img) 
                   VALUES  ('".$_POST['title']."','".$_POST['descp']."','".$img."')";

    $fire=mysqli_query($conn,$query);
if($fire){

    echo "<script>alert('Success !!!');window.location.href='slider.php';</script>";
}
else{
    echo "Not OKKKKK !!!!!!";
}
...
if(isset($_GET['deleteid']))
    {
        // echo "okk";
       $query="DELETE FROM slider where id='".$_GET['deleteid']."'";
        ...
        $fire=mysqli_query($conn,$query);
        if($fire){
            echo "<script>alert('delete data');window.location.href='slider.php';</script>";
        }
        else
        {
            echo "error";
   ...
   if(isset($_POST['title1']))
{
    ...
    unlink($path1);
      ...
      $query="UPDATE slider SET title='".$_POST['title1']."',descp='".$_POST['descp']."',img='".$img."' WHERE id='".$_POST['id']."'";

       $fire=mysqli_query($conn,$query);
        if($fire){
            echo "<script>alert('update data');window.location.href='slider.php';</script>";
        }
        else
        {
            echo "error";
